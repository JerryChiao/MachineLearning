...
def get_best_split(data_array, ops=(1, 4)):
    """
    get the best split point based on variance
    :param data_array:
    :param ops:
    :return:
    """
...
def gbdt(data_array, num_iter, ops=(1, 4)):
    ...
    for i in xrange(num_iter):
        if i == 0:
            ...
            # this is the gradient of 1/2(y - y_hat)^2
            r = y - np.array(yHat)
            data_array = np.hstack((x, r))
            curr_err =  np.mean( data_array[:, -1] ** 2)
            if abs(curr_err - pre_err) < epsilon:
                break
            pre_err = curr_err
            tree = buildTree(data_array, ops)
            list_trees.append(tree)
...
def print_tree(tree, indent=''):
    ...
    if tree.results != None:
        ...
        # Print the branches
        print indent + 'T->',
        print_tree(tree.gb, indent + '  ')
        print indent + 'F->',
        print_tree(tree.lb, indent + '  ')
...
if __name__ == '__main__':
    ...
    data = loadDataSet('tree_test.txt')
    ...
    i = 0
    for tree in gbdt_results:
        print_tree(tree)
        drawtree(tree, jpeg="gbdt_%d.jpg" %i)
        i+=1
